...
function deriveProteinsForTemplate(tpl){
  const p = (tpl.shopping_core?.protein || []);
  const tp = (tpl.tags?.proteins || []);
  const primary = p[0] || tp[0] || "—";
  const secondary = p[1] || (tp.length>1 ? tp[1] : "—");
  return { primary:{label:String(primary)}, secondary:{label:String(secondary)} };
}
function renderTemplate(tpl, flavorKey, mode, source, flavorsDict, tweaks=null){
  ...
  const proteins = tpl.proteins || deriveProteinsForTemplate(tpl);
  ...
  p.innerHTML = `
    ...
    <div class="grid3">
      ...
      <div class="box">
        ...
        <ul>${(tpl.base_blocks||[]).map(b=>`<li>${b.label}</li>`).join("") || "<li class='muted'>See outputs above.</li>"}</ul>
    ...
    <div class="grid3" style="margin-top:12px">
      ...
      <div class="box">
        ...
        <ul>${(tpl.weekday_assembly?.derived_meals||tpl.derived_meals||[]).map(m=>`<li><strong>${m.title}</strong>: ${m.how.join(" • ")}</li>`).join("")}</ul>
      </div>
    </div>
