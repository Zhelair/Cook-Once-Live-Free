...
async function openSaved(){
  const saved = getSaved();
  if (!saved.length){
    $("#savedBox").innerHTML = `<div class="meta">No saved recipes yet. Hit ⭐ on something you like.</div>`;
...
async function getRecipe(mode){
  ...
  if (mode === "match" && pantry.length === 0){
    toast("Add at least 1 pantry item first (onion, rice, chicken…).");
    return;
  }
  toast("Fetching a recipe…");
...
async function init(){
  ...
  setupEvents();
  toast("Ready. Add pantry → pick recipe.");
}
