...
function getPortionsSafe(tpl){
  const p = (tpl && tpl.portions) ? tpl.portions : null;
  return {
    main_meals: p?.main_meals ?? null,
    extras: p?.extras ?? null,
  };
}

function renderTemplate(tpl, flavorKey, mode, source, flavorsDict, tweaks=null){
  const flavorsSafe = flavorsDict || window.__flavors || {};
  window.__flavors = flavorsSafe;
  const flavor = flavorsSafe[flavorKey] || {name: flavorKey, adds:[]};
  const o = $("#output");
  const p = $("#plan");

  const fo = tpl.flavor_overrides?.[flavorKey];
  const season = fo?.season_chicken || flavor.adds || [];
  // Safe yield (old templates had tpl.portions; new ones may not)
  const portions = getPortionsSafe(tpl);
  const yieldHtml = (portions.main_meals==null && portions.extras==null)
    ? ""
    : `${yieldHtml}`;
