...
function detectBasePrepType(meal){
  const ings = mealIngredients(meal);
  const t = ings.join(" ");
  // very lightweight protein/base detection (best effort)
  ...
  if (t.includes("fish") || t.includes("salmon") || t.includes("tuna") || t.includes("cod")) return "fish";
  if (t.includes("lentil") || t.includes("chickpea") || t.includes("bean")) return "legumes";
...
function derivedTemplatesFor(type){
  // Assembly templates (NOT full recipes)
  // Each template: title + bullets
  const common = [
    {title:"🥗 Quick fresh side", bullets:["add salad / cucumber / tomato", "or yogurt / pickles (fast & Balkan-approved)"]},
    {title:"🍚 Easy carbs", bullets:["rice / pasta / potatoes", "cook fresh in 10–15 min while base reheats"]},
  ];

  const map = {
    chicken: [
      {title:"🍽 Plate meal", bullets:["reheat chicken base", "add roasted veg + carbs"]},
      {title:"🥪 Sandwich / wrap", bullets:["shred chicken", "add sauce + greens", "wrap/bread = instant dinner"]},
      {title:"🍲 Soup-ish bowl", bullets:["add broth/water + veg", "drop in chicken", "done in one pot"]},
      {title:"🥣 Rice bowl", bullets:["chicken + rice", "add quick veg / sauce", "meal-prep friendly"]},
      ...common
    ],
    beef: [
      {title:"🍽 Plate meal", bullets:["reheat stew / beef base", "add potatoes/rice"]},
      {title:"🍲 Thinner soup version", bullets:["add a bit of water/stock", "add frozen veg", "simmer 10 min"]},
      {title:"🥪 Loaded sandwich", bullets:["thick beef base", "bread + pickles", "minimal cooking"]},
      {title:"🥣 Bowl", bullets:["beef base + rice", "add fresh greens", "optional yogurt"]},
      ...common
    ],
    pork: [
      {title:"🍽 Plate meal", bullets:["reheat pork base", "add cabbage/potato", "one-pan if possible"]},
      {title:"🍲 Soup-ish", bullets:["pork bits + broth", "veg + noodles/rice", "10–15 min"]},
      {title:"🥪 Sandwich", bullets:["slice/shred", "add mustard/pickles", "bread = done"]},
      ...common
    ],
    fish: [
      {title:"🍽 Plate meal", bullets:["reheat gently (avoid drying)", "add potatoes/rice + lemon"]},
      {title:"🥗 Salad bowl", bullets:["fish + greens", "add beans/corn", "olive oil + lemon"]},
      {title:"🥪 Quick toast", bullets:["fish + bread", "add cucumber/tomato", "fast dinner"]},
      ...common
    ],
    legumes: [
      {title:"🍽 Plate meal", bullets:["lentils/beans base", "add rice/pasta", "top with yogurt (if ok)"]},
      {title:"🍲 Soup", bullets:["thin with water/stock", "add veg", "simmer 10 min"]},
      {title:"🥗 Bowl", bullets:["base + greens", "add olive oil/lemon", "very meal-prep friendly"]},
      ...common
    ],
    eggs: [
      {title:"🥪 Sandwich", bullets:["eggs + bread", "add cheese/veg", "fast & cheap"]},
      {title:"🍚 Fried rice vibe", bullets:["rice + eggs", "add veg", "one-pan"]},
      ...common
    ],
    dessert: [
      {title:"🍰 Portions", bullets:["portion into containers", "snack/dessert through the week"]},
      ...common
    ],
    mixed: [
      {title:"🍽 Plate meal", bullets:["reheat base", "add carbs + salad"]},
      {title:"🍲 Soup-ish", bullets:["add broth/water + veg", "simmer 10 min"]},
      {title:"🥪 Sandwich", bullets:["thick base", "bread + fresh greens", "done"]},
      ...common
    ]
  };
  return map[type] || map.mixed;
}

function renderWeeklyPlan(meal, mode){
  const wp = $("#weeklyPlanBox");
  if (!wp) return;

  const wm = estimateWeeklyMeta(meal);
  const type = detectBasePrepType(meal);
  const templates = derivedTemplatesFor(type);

  const baseTitle = (mode === "match") ? "Base prep (from your pantry)" : "Base prep (random base)";
  const sundayBullets = [
    `Cook <strong>${meal.strMeal}</strong> as your base dish (scale up for leftovers)`,
    "Aim for 4–6 portions (containers)",
    ($("#batchToggle").checked ? "Batch mode ON: prefer one-pot/one-tray approach" : "Batch mode OFF")
  ];

  // A simple 2nd prep suggestion depending on type
  let secondPrep = "Tray of roasted vegetables (same oven)";
  if (type === "chicken") secondPrep = "Broth / soup starter (use leftovers)";
  if (type === "beef") secondPrep = "Extra roasted veg or a pot of rice";
  if (type === "fish") secondPrep = "Chop salad base + lemon sauce";
  if (type === "legumes") secondPrep = "Cook rice + chop salad base";

  const weekdayBullets = [
    "Reheat base (10–15 min)",
    "Add quick carbs (rice/pasta/potato)",
    "Add fresh side (salad/yogurt/pickles)"
  ];

  const variationsHtml = templates.slice(0,4).map(t => `
    <div class="week-card">
      <h4>${t.title}</h4>
  ...
  wp.innerHTML = `
    <div class="week-badges">
      <span class="badge">${baseTitle}</span>
      <span class="badge">Feeds: ${wm.feeds}</span>
      <span class="badge">Keeps: ${wm.keeps}</span>
      <span class="badge">${wm.freezer}</span>
    </div>

    <div class="week-grid">
      <div class="week-card">
        <h4>🔥 Sunday (cook once)</h4>
        <ul>${sundayBullets.map(x=>`<li>${x}</li>`).join("")}</ul>
        <div class="meta" style="margin-top:8px"><strong>Optional 2nd prep:</strong> ${secondPrep}</div>
      </div>

      <div class="week-card">
        <h4>🍽 Weekdays (10–15 min)</h4>
        <ul>${weekdayBullets.map(x=>`<li>${x}</li>`).join("")}</ul>
        <div class="meta" style="margin-top:8px"><strong>Tip:</strong> you’re not “cooking daily” — you’re assembling.</div>
      </div>

      <div class="week-card">
        <h4>✨ Variations (assembly templates)</h4>
        <div class="meta">Pick 2–3 styles for the week:</div>
        <ul>
          <li>Plate meals</li>
          <li>Soup-ish bowls</li>
          <li>Sandwich/wraps</li>
          <li>Rice/protein bowls</li>
        </ul>
      </div>
    </div>

    <div class="plan-section">
      <h4>✅ Derived meals (click recipe below for base instructions)</h4>
      <div class="week-grid">${variationsHtml}</div>
    </div>

    <div class="meta" style="margin-top:10px">
      <strong>Cook Once principle:</strong> 1 base recipe → many meals. The recipe below is your base prep instructions.
    </div>
  `;
}
