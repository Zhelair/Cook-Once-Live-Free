...
function deriveProteins(coreShop, tags){
  const p = (coreShop?.protein || []);
  const tp = (tags?.proteins || []);
  const primary = p[0] || tp[0] || "—";
  const secondary = p[1] || (tp.length>1 ? tp[1] : "—");
  return {
    primary: { label: String(primary) },
    secondary: { label: String(secondary) }
  };
}
...
function renderTemplate(tpl, flavorKey, mode, source, flavorsDict, tweaks=null){
  ...
  const coreShop = tpl.shopping_core || {protein:[], veg:[], pantry:[], optional:[]};
  const proteins = tpl.proteins || deriveProteins(coreShop, tpl.tags);
  ...
  p.innerHTML = `
    ...
    <div class="grid3">
      ...
      <div class="box">
        <h4>🔥 Flavor pack</h4>
        <div class="muted"><strong>Season with:</strong></div>
        <ul>${li(season)}</ul>
